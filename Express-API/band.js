...
const editBand = "update band SET name=? WHERE band_id=?"
    ...
    editBand(params, db) {
        return new Promise((resolve, reject) => {
            try {
                ...
                db.query(editBand, [params.name, params.band_id]).then(rows => {
                    resolve({ "success": "Successfully edited band" })
                })
            } catch (err) {
                reject({ "error": "Error editing band" })
            }
        })
    }
}
